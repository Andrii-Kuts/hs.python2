...
class PesunBot:
    def __init__(self):
        ...
        self.app.add_handler(CommandHandler("statistics", self.handle_statistics, filters.ALL))
    ...
    async def init_bot(self, app: Application):
        ...
        commands = [
            ...
            telegram.BotCommand("/statistics", "Builds an app with statistics and sends a link to it"),
        ]
    ...
    async def handle_import_file(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        response = await response.edit_text(f"⏱️  Parsing...")
        dataset = None
        async for parse_result in parse_zip(file):
            if parse_result.isDone():
                ...
                response = await response.edit_text(f"⏱️  Parsing file {parse_result.file_number}/{parse_result.total_files}")
        if dataset is None:
    ...
    async def handle_statistics(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        response = await response.edit_text(f"✅  View statistics here:\n\nhttp://0.0.0.0:8050{plotterData.path}")
